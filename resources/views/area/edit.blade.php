            ...
            <div class="bg-white p-6 shadow sm:rounded-lg">
                <form action="{{ route('areas.update', $area->idArea) }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="nomArea" class="w-full border rounded p-2" value="{{ old('nomArea', $area->nomArea) }}" required>
                        @error('nomArea') <p class="text-red-500 text-sm">{{ $message }}</p> @enderror
                    </div>

                    <div class="mb-4">
                        <label class="block text-gray-700">Salario</label>
                        <input type="number" step="0.01" name="salario" class="w-full border rounded p-2" value="{{ old('salario', $area->salario) }}" required>
